...
- **RC03:** Tratar envio e processamento como tarefas assíncronas. Cada vídeo deve ter um estado informado através de texto, como "enviando", "aguardando", "processando", "pausado", "pronto" ou "erro", acompanhado da etapa em execução. Porcentagem e previsão de término só devem aparecer quando houver dados confiáveis. O usuário deve poder sair, retornar e retomar uma tarefa interrompida sem perder o trabalho. Derivada de C02, C04 e C05.
...
- **RC05:** Manter a decisão editorial com o usuáriol, nosso sistema não abrange edição de vídeo no escopo, portanto isso não será incorporado na interface. A interface deve permitir revisar, selecionar e descartar os resultados da IA e explicar qualquer porcentagem de confiança, bem como limites, falhas parciais e partes do vídeo que não puderam ser analisadas. Um resultado vazio não deve ser apresentado como prova de que a partida não teve acontecimentos relevantes. Derivada de C04, C06 e C07.
- **RC06:** Não é necessário oferecer edição dos cortes nesta interface. O usuário deve poder definir configurações básicas de inferência em uma área dedicada junto à entrada dos vídeos. Essa área pode reunir, por exemplo, os tipos de evento a detectar, o nível mínimo de confiança e o contexto temporal dos cortes, com valores padrão e explicações curtas. Derivada de C01, C02, C03, C04 e C05, além da delimitação definida nas seções 7.4 e 11 da Entrega 1.
- **RC07:** Usar o vocabulário do trabalho do editor, com termos como "partida", "vídeo enviado", "highlight", "início", "fim", "duração" e "baixar corte(s)". O sistema deve preencher automaticamente dados que já conhece, como arquivo de origem, tipo de evento e timecode, sem expor termos internos como "inferência" ou exigir nomes genéricos como "Clip 1". Derivada de C01, C02, C04 e C05.
...
- **RC10:** Disponibilizar um histórico das análises realizadas, com busca por partida, data e estado. Cada registro deve mostrar o vídeos de origem, os resultados gerados, a data, o identificador do processamento e, quando houver, o motivo da falha. Antes de iniciar uma nova análise para um vídeo já processado, a interface deve identificar o reprocessamento, informar o custo previsto e pedir confirmação. Derivada de C02 e C04.
- **RC11:** Garantir que envio, revisão e download possam ser realizados pelo teclado, segurar ⌘ mostra a combinação de teclas necessárias para realizar a ação de um botão (ex: S aparece ao lado do botão de salvar ao segurar a tecla ⌘). Todos os controles devem ter nome acessível e foco visível. Estados e erros devem aparecer em texto, sem depender apenas de cor, ícone, animação ou arraste. Derivada das limitações de acessibilidade observadas em C01, C02, C03, C04 e C05.
- **RC12:** Priorizar uma interface web para computador (electron?), adequada à inspeção de vídeos longos em tela ampla e sem dependência de um sistema operacional específico. O protótipo deve considerar upload demorado, interrupções e retorno posterior ao trabalho. Derivada de C04 e da decisão ainda aberta registrada na seção 5.2 da Entrega 1.
